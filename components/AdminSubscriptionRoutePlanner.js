...
const formatWhatsAppRouteMessage = (routePlan, currency = "INR") => {
  ...
  if (Number(routePlan.returnDistanceKm || 0) > 0) {
    lines.push(`${routePlan.stops.length + 1}. *Name:* Return to pickup point`);
    lines.push("   *Phone:* -");
    lines.push(`   *Address:* ${routePlan.originAddress || "-"}`);
    lines.push("   *Location:* -");
    lines.push(`   *Distance:* ${Number(routePlan.returnDistanceKm || 0).toFixed(1)} km`);
    lines.push("");
  }

  lines.push(`*Total Distance Travelled:* ${Number(routePlan.totalDistanceKm || 0).toFixed(1)} km`);
  lines.push(`*Total Payout:* ${formatCurrency(currency, routePlan.driverPayout || 0)}`);
...
export default function AdminSubscriptionRoutePlanner({
  ...
  const handleCopyWhatsAppText = async (routePlan) => {
    ...
    const copied = await copyToClipboard(formatWhatsAppRouteMessage(routePlan, currency));
  ...
  return (
    <section className="rounded-2xl bg-base-100 p-5 shadow-md">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold">Upcoming delivery runs</h2>
          <p className="text-sm opacity-70">
            Committed delivery runs are recalculated automatically when deliveries change.
          </p>
        </div>
        <div className="badge badge-outline">
          {`${routeSnapshots.length} run${routeSnapshots.length === 1 ? "" : "s"}`}
        </div>
      </div>
      ...
      <div className="mt-4 space-y-4">
        ...
        {routeSnapshots.map((routePlan) => {
          const isPrimaryRoute = currentRoute?.deliveryDate === routePlan.deliveryDate;
